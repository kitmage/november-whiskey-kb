...
- **Output**
  ...
  - With `style="details"`, the root list also receives `kb-article-titles-details`; parent items with children render a `<details class="kb-article-details">` wrapper and `<summary class="kb-article-summary">` title, active-parent `<details>` elements render open on page load, and childless top-level parent items receive a `childless` class.
